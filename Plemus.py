...
class Windows:
    def __init__(self) -> None:
        ...
        self.titileLabel = Label(self.playFrame, text="暂无歌曲", font=("微软雅黑", 20))
        self.authorLabel = Label(self.playFrame, text="未知作家")
        ...
        self.loopPlaybackButton = Button(self.settingFrame, text="单曲循环", command=self.loopPlayback)
        self.playOrStopButton = Button(self.settingFrame, text="播放", command=self.stopOrPlay)
        ...
        self.starting = False
        self.musicloopPlayback = 0 # 0: 不循环; 1: 单曲循环; 2: 列表循环
    ...
    def loopPlayback(self):
        ...
        if self.musicloopPlayback == 0:
            self.loopPlaybackMessage["text"] = "已取消循环"
            self.loopPlaybackButton["text"] = "单曲循环"
        elif self.musicloopPlayback == 1:
            self.loopPlaybackMessage["text"] = "已切换至单曲循环"
            self.loopPlaybackButton["text"] = "列表循环"
        elif self.musicloopPlayback == 2:
            self.loopPlaybackMessage["text"] = "已切换至列表循环"
            self.loopPlaybackButton["text"] = "取消循环"
    ...
    def stopOrPlay(self):
        if self.playOrStop == True:
            ...
            if self.timeSeconds >= self.songSeconds:
                self.playSong("./resource/music")
            try:
                pygame.mixer.music.play(0, self.timeSeconds)
            except pygame.error: return

            self.playOrStop = True
            self.playOrStopButton["text"] = "暂停"
    ...
    async def getMusicList(self):
        ...
        for i in self.songsTable.keys():
            ...
            self.authorList.insert(END, self.songsTable[i]["author"])
